<?php
    class OutsideRecordD extends OutsideRecordAbstract
    {
            ...
            if( !empty( $this->is_closed ) )
            {
                if( !empty( $is_cached ) )
                {
                    if( 'freight' == $this->section )
                    {
                        ...
                        Db::i()->update( $this->section, array( 'deleted' => 1, 'status' => 'canceled' ), array( 'id' => $this->id ) );
                        Db::i()->update( $this->section . '_outside', array( 'deleted' => 1 ), array(
                            'source' => $this->source,
                    ...
                    {
                        Db::i()->delete( $this->section . '_outside', array(
                            'source' => $this->source,
                            'record_id' => $this->record_id
                        ) );
                    }
                    ...
                    {
                        $this->status = self::DELETE_ERROR;

                        @Tools::report( Db::i()->displayLastQuery(), 'Della Import Record Error' );
                    }
                }
                else
                {
