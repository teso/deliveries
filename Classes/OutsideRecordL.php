<?php
    class OutsideRecordL extends OutsideRecordAbstract
    {
        public function save()
        {
            ...
            if( /*('freight' == $this->section && (empty( $this->owner_id ) || empty( $this->group_id ) || empty( $this->email_id ) || $has_records))
                ...
                || ('vehicle' == $this->section && !isMail( $this->email )) )
            {
                if( 'freight' == $this->section )
                {
            ...
            else
            {
                /*if( 'freight' == $this->section )
                {
                    ...
                    if( !empty( Db::i()->error ) )
                    {
                        @Tools::report( Db::i()->displayLastQuery(), 'Lardi Import Record Error' );
                    }
                }*/
            ...
            $is_cached_stats = $this->_isCachedStats( $this->record_id, $this->source );
            $values = $this->compact( $this->records_outside_fields );

            if( !empty( $is_cached_stats ) )
            {
                unset( $values['source'] );
                unset( $values['record_id'] );

                Db::i()->update( $this->section . '_outside_stats', $values, array(
                    'source' => $this->source,
                    'record_id' => $this->record_id
                ) );
            }
            ...
            {
                Db::i()->insert( $this->section . '_outside_stats', $values );
            }
        }
